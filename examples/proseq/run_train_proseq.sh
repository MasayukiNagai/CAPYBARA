#!/bin/bash
#SBATCH --job-name=train_capy_proseq
...
# Train CAPY on PRO-seq / Rogers-timecourse gene-body data.
#
# Usage:
#   sbatch examples/proseq/run_train_proseq.sh [timestamp] [params] [treatment] [fold] [gpu] [stage]
...
script="${REPO_ROOT}/examples/proseq/train_proseq.py"
...
gpu="${5:-0}"
stage="${6:-train}"   # train, finetune, both
