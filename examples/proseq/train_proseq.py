...
"""Train CAPY on processed PRO-seq / gene-body (Rogers-timecourse) data.

Mirrors ``examples/procap/train_capy.py`` but wires in the PRO-seq data path:
``ProSeqFilesConfig`` (Rogers file layout) + ``ProSeqDataModule`` (strand-aware,
single-strand, TSS-anchored extraction). The CAPY model, losses, and training
loop (``examples/procap/train_utils.py``) are reused unchanged.

Example:
    .venv/bin/python examples/proseq/train_proseq.py \
        --proj_dir /grid/koo/home/ykang/elongation/CAPYBARA/runs \
        --params configs/proseq_default.yaml \
        --treatment GSM8306530_0m --fold fold1 --stage train --device gpu
"""
...
PROCAP_DIR = REPO_ROOT / "examples" / "procap"           # reused train_utils + performance_metrics
# Order matters: proseq first so `import data` resolves to OUR data.py, then
# procap (for train_utils / performance_metrics), then the repo root (capybara).
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train CAPY on processed PRO-seq data.")
    parser.add_argument("--proj_dir", type=Path, required=True, help="Output root; trained models go under <proj_dir>/models/...")
    parser.add_argument("--params", type=Path, default=REPO_ROOT / "configs" / "proseq_default.yaml")
...
def load_source_model(files: ProSeqFilesConfig, device: torch.device) -> nn.Module:
...
def run_train_stage(*, args: argparse.Namespace, params: dict[str, Any], device: torch.device) -> ProSeqFilesConfig:
    ...
    metadata = {"model_name": "capy", "params": params, "file_config": config_dict}
    model = CAPY(model_cfg)
    print(f"Training CAPY (PRO-seq) on {device}; outputs: {files.model_dir}", flush=True)
...
def run_finetune_stage(
    *,
    ...
    print(f"Fine-tuning mode: {fine_tune_cfg['mode']}; tuned outputs: {target_files.model_dir}", flush=True)
    print_model_summary(model)
