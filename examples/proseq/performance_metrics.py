### PRO-seq copy of examples/procap/performance_metrics.py.
### IDENTICAL math; the only change is that the integer-count asserts in
### compute_performance_metrics() are disabled, because PRO-seq / Rogers BigWig
### signal is fractional (normalized). This matches ProCapNet's own
### src/2_train_models/performance_metrics.py, where those asserts are commented.
### train_proseq.py makes train_utils.validate() use this module (sys.path order
### + an explicit rebind).
###
...
def compute_performance_metrics(
    ...
    # Disabled for PRO-seq: Rogers BigWig signal is fractional (normalized), so
    # counts are not integers. The multinomial NLL via gammaln is well-defined for
    # non-integer "counts", so all metric values are unchanged. (ProCapNet's
    # performance_metrics.py comments these out for the same reason.)
