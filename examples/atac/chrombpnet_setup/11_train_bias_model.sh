...
if [[ -d "${BIAS_MODEL_DIR}" ]] && [[ -n "$(ls -A "${BIAS_MODEL_DIR}" 2>/dev/null)" ]] && [[ "${OVERWRITE:-0}" != "1" ]]; then
  die "Output exists: ${BIAS_MODEL_DIR}. Set OVERWRITE=1 to allow rerun into existing directory."
fi
ensure_dir "${BIAS_MODEL_DIR}"
...
CONTAINER_BIAS_MODEL_DIR="$(readlink -f "${BIAS_MODEL_DIR}")"

add_bind "$(readlink -f "${BAM_DIR}")"
add_bind "$(readlink -f "${DATA_DIR}")"
add_bind "$(readlink -f "${GENOME_DIR}")"
add_bind "${CONTAINER_BIAS_MODEL_DIR}"

# Train bias model using chrombpnet.
apptainer_cmd=(
  apptainer exec
  "${APPTAINER_BIND_ARGS[@]}"
  --nv
