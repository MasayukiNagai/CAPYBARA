...
#
# Tier-B for the CAPY accessibility (nobias) model: contribution scores (profile)
# -> TF-MoDISco. Recovers TF motifs to compare vs ChromBPNet.
# Usage: sbatch submit_attribution_nobias.sh [timestamp] [method] [cell_type] [fold] [bias_timestamp] [gpu]
...
echo "REPO_ROOT=${REPO_ROOT} proj_dir=${proj_dir} cell=${cell_type} fold=${fold} ts=${timestamp} bias=${bias_timestamp} method=${method}"
...
"${DIR}/run_modisco.sh" \
  "${attr_dir}/${cell_type}.fold_${fold}.profile_scores.h5" \
  "${attr_dir}/modisco/profile"
