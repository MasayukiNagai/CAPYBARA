#!/bin/bash
#SBATCH --job-name=eval_bias_capy
...
#
# Tier-A evaluation only (no retrain) of an already-trained CAPY bias checkpoint.
# Usage: sbatch submit_bias_eval.sh <timestamp> [cell_type] [fold] [split] [gpu]
set -euo pipefail
...
timestamp="${1:?usage: sbatch submit_bias_eval.sh <timestamp> [cell_type] [fold] [split] [gpu]}"
cell_type="${2:-K562}"
fold="${3:-0}"
split="${4:-test}"
gpu="${5:-0}"
...
echo "proj_dir=${proj_dir}  cell_type=${cell_type}  fold=${fold}  split=${split}  timestamp=${timestamp}"
