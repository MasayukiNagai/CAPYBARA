...
def require_training_dependencies() -> None:
    ...
    if missing:
        raise ImportError(
            ...
            + ", ".join(missing)
            + ". Install them in the environment before running training scripts."
        )
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            self.run = wandb.init(
                project=wandb_cfg.get("project", "capybara"),
...
def append_count_finetune_metrics(path: str | Path, row: dict[str, Any], *, write_header: bool) -> None:
    append_metrics_row(path, row, write_header=write_header, columns=COUNT_FINETUNE_METRICS_COLUMNS)
...
def run_training_loop(
    *,
    ...
    def validate_and_log(*, epoch: int, iteration: int, train_metrics: dict[str, float], start: float) -> None:
    ...
    try:
        for epoch in range(max_epochs):
            ...
            for batch in train_loader:
                ...
                if validation_iter is not None and iteration % validation_iter == 0:
                    validate_and_log(epoch=epoch, iteration=iteration, train_metrics=last_train_metrics, start=start)
            ...
            if validation_iter is None:
                validate_and_log(epoch=epoch, iteration=iteration, train_metrics=epoch_averages, start=start)
...
def train_model(
    *,
    ...
    run_training_loop(
        ...
        set_train_mode_fn=lambda m: m.train(),
...
def finetune_count_head(
    *,
    ...
    return run_training_loop(
        ...
        validate_fn=lambda m, vl, d: validate_count_only(m, vl, d),
        set_train_mode_fn=lambda m: set_count_finetune_train_mode(m, model_name, mode),
