...
def multinomial_log_probs(
    category_log_probs: np.ndarray,
    trials: np.ndarray,
    query_counts: np.ndarray,
    return_cross_entropy: bool = True,
) -> tuple[np.ndarray, np.ndarray] | np.ndarray:
    """Compute multinomial log probabilities and optional cross entropy."""
    ...
    if return_cross_entropy:
        cross_ent = np.divide(
            -log_prob_pows_sum,
            trials,
            out=np.full_like(log_prob_pows_sum, np.nan, dtype=float),
            where=(trials != 0),
        )
...
def profile_multinomial_nll(
    true_profs: np.ndarray,
    log_pred_profs: np.ndarray,
    true_counts: np.ndarray,
    prof_smooth_kernel_sigma: float,
    prof_smooth_kernel_width: int,
    smooth_pred_profs: bool = False,
    return_cross_entropy: bool = True,
    batch_size: int = 200,
) -> tuple[np.ndarray, np.ndarray] | np.ndarray:
    """Compute per-sample, per-task profile multinomial negative log likelihoods."""
    ...
    for start in range(0, num_samples, batch_size):
        ...
        result_batch = multinomial_log_probs(
            log_pred_profs_batch, true_counts_batch, true_profs_batch, return_cross_entropy
        )
        if return_cross_entropy:
...
def _kl_divergence(probs1: np.ndarray, probs2: np.ndarray) -> np.ndarray:
    """Compute KL divergence along the final axis."""
    quot = np.divide(
        probs1, probs2, out=np.ones_like(probs1), where=((probs1 != 0) & (probs2 != 0))
    )
    return np.sum(probs1 * np.log(quot), axis=-1)


def jensen_shannon_distance(probs1: np.ndarray, probs2: np.ndarray) -> np.ndarray:
    """Compute Jensen-Shannon divergence along the final axis."""
...
def profile_jsd(
    true_prof_probs: np.ndarray,
    pred_prof_probs: np.ndarray,
    prof_smooth_kernel_sigma: float,
    prof_smooth_kernel_width: int,
    smooth_true_profs: bool = True,
    smooth_pred_profs: bool = False,
    batch_size: int = 200,
) -> np.ndarray:
    """Compute per-sample, per-task profile Jensen-Shannon divergence."""
    ...
    for start in range(0, num_samples, batch_size):
        ...
        finite = np.isfinite(jsd_batch)
        finite_count = np.sum(finite, axis=-1)
        jsd_batch_mean = np.divide(
            np.nansum(jsd_batch, axis=-1),
            finite_count,
            out=np.full(jsd_batch.shape[:-1], np.nan, dtype=float),
...
def pearson_corr(arr1: np.ndarray, arr2: np.ndarray) -> np.ndarray:
    """Compute Pearson correlation along the final axis."""
...
def average_ranks(arr: np.ndarray) -> np.ndarray:
    """Assign average ranks along the final axis, averaging tied values."""
...
def spearman_corr(arr1: np.ndarray, arr2: np.ndarray) -> np.ndarray:
    """Compute Spearman correlation along the final axis."""
    return pearson_corr(average_ranks(arr1), average_ranks(arr2))


def mean_squared_error(arr1: np.ndarray, arr2: np.ndarray) -> np.ndarray:
    """Compute mean squared error along the final axis."""
    return np.mean(np.square(arr1 - arr2), axis=-1)


def r2_score(arr1: np.ndarray, arr2: np.ndarray) -> np.ndarray:
    """Compute R-squared scores along the final axis."""
...
def profile_corr_mse(
    ...
    """Compute profile Pearson, Spearman, and MSE metrics."""
...
def count_corr_mse(
    log_true_total_counts: np.ndarray, log_pred_total_counts: np.ndarray
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Compute count Pearson, Spearman, and MSE metrics."""
    pears, spear, mse, _ = count_corr_mse_r2(log_true_total_counts, log_pred_total_counts)
    return pears, spear, mse


def count_corr_mse_r2(
    log_true_total_counts: np.ndarray, log_pred_total_counts: np.ndarray
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Compute count Pearson, Spearman, MSE, and R-squared metrics."""
    num_tasks = log_true_total_counts.shape[1]
    log_true_total_counts = np.reshape(np.swapaxes(log_true_total_counts, 0, 1), (num_tasks, -1))
    log_pred_total_counts = np.reshape(np.swapaxes(log_pred_total_counts, 0, 1), (num_tasks, -1))
...
def compute_performance_metrics(
    ...
    prof_smooth_kernel_sigma: float = 7,
    prof_smooth_kernel_width: int = 81,
    smooth_true_profs: bool = True,
    smooth_pred_profs: bool = False,
) -> dict[str, np.ndarray]:
    """Compute all profile and count performance metrics for a prediction set."""
